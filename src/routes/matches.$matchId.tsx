  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-20 flex h-14 items-center gap-4 border-b border-border bg-surface/80 px-4 backdrop-blur">
        ...
        <span className="text-mono text-[10px] text-muted-foreground">/ <Link to="/matches" className="hover:text-foreground">Матчи</Link> / {match.name}</span>
      ...
      <div className="mx-auto max-w-5xl p-6">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <div className="label-eyebrow text-[10px] text-muted-foreground">{tournament?.name}</div>
            <h1 className="text-xl font-bold leading-tight">{match.name}</h1>
          </div>
          <div className="text-mono ml-auto rounded-sm border border-border bg-surface-2 px-2 py-1 text-[10px] text-muted-foreground">
            ...
            return (
              <Link key={g.id} to="/games/$gameId" params={{ gameId: g.id }}
                ...
                <div className="p-3">
                  ...
                  <div className="text-mono mt-1 text-[10px] text-muted-foreground">Длительность {mm(g.durationSec)}</div>
                </div>
              </Link>
            );
          })}
