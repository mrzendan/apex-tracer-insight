  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-surface px-4">
        ...
        <span className="text-mono text-xs text-muted-foreground">{flow.nodes.length} blocks · {flow.edges.length} arrows</span>
        <div className="ml-2 flex items-center gap-1 rounded-sm border border-border bg-surface-2 p-0.5">
          {flows.map((f) => (
            <button key={f.id} onClick={() => setPrefs((p) => ({ ...p, current: f.id }))}
              className={`px-2 py-0.5 text-xs uppercase tracking-wider rounded-sm ${prefs.current === f.id ? "bg-primary/20 text-primary" : "text-muted-foreground hover:text-foreground"}`}>
              {f.name}
            </button>
          ))}
        </div>
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="ml-2 flex items-center gap-1">
            {(["process", "decision", "data", "io", "start", "end"] as NodeKind[]).map((k) => (
              <button key={k} onClick={() => addNode(k)} className="rounded-sm border border-border bg-surface-2 px-1.5 py-1 text-xs uppercase tracking-wider hover:bg-muted">+ {k}</button>
            ))}
          </div>
          <button onClick={() => { setFlows(seed()); setSelected(null); }} className="rounded-sm border border-border bg-surface-2 px-2 py-1 text-xs uppercase tracking-wider hover:bg-muted">Reset</button>
          ...
          <button onClick={() => { setZoom(1); setPan({ x: 20, y: 20 }); }} className="rounded-sm border border-border bg-surface-2 px-2 py-1 text-xs hover:bg-muted">Fit</button>
        </div>
      </header>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <aside className="flex w-[300px] shrink-0 flex-col border-l border-border bg-surface">
          ...
          <div className="flex-1 overflow-y-auto p-3 text-xs">
            <div className="label-eyebrow mb-1 text-xs">Diagram</div>
            <div className="mb-3 rounded-sm border border-border bg-background p-2 text-xs">
            ...
            {selNode && (
              <div className="space-y-3">
                ...
                <div>
                  <div className="label-eyebrow mb-1 text-xs">Shape</div>
                  <div className="flex flex-wrap gap-1">
                    {(["start", "process", "decision", "data", "io", "end"] as NodeKind[]).map((k) => (
                      <button key={k} onClick={() => setNodeKind(selNode.id, k)}
                        className={`rounded-sm border px-1.5 py-0.5 text-xs uppercase tracking-wider ${selNode.kind === k ? "border-primary/40 bg-primary/15 text-primary" : "border-border bg-background hover:bg-muted"}`}>
                        {k}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => { if (connect === selNode.id) setConnect(null); else setConnect(selNode.id); }}
                    className={`flex-1 rounded-sm border px-2 py-1 text-xs uppercase ${connect === selNode.id ? "border-primary/40 bg-primary/15 text-primary" : "border-border bg-background hover:bg-muted"}`}>
                    {connect === selNode.id ? "Cancel connect" : "Connect →"}
                  </button>
                  <button onClick={() => { deleteNode(selNode.id); setSelected(null); }} className="flex-1 rounded-sm border border-destructive/40 bg-destructive/10 px-2 py-1 text-xs uppercase text-destructive hover:bg-destructive/20">Delete</button>
                </div>
              </div>
            )}
          </div>
