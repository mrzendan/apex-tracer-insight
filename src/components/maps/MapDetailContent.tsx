...
export function MapDetailContent({
  ...
  const [teamId, setTeamId] = useState<string>(initialTeam ?? teams[0]?.id ?? "");
  useEffect(() => {
    if (initialTeam && initialTeam !== teamId) setTeamId(initialTeam);
  ...
  if (!map) {
    return (
      <div className="p-6 text-sm">
        Map not found. <Link to={backTo} className="text-primary underline">{backLabel}</Link>
      </div>
    );
  }
  ...
  const heatColor = (t: number) => {
    if (t < 0.5) {
      const k = t / 0.5;
      return `rgb(255, ${Math.round(220 - k * 90)}, ${Math.round(60 - k * 60)})`;
    }
    const k = (t - 0.5) / 0.5;
    return `rgb(${Math.round(255 - k * 40)}, ${Math.round(130 - k * 110)}, ${Math.round(0 + k * 10)})`;
  };
            ...
            <div className="relative w-full overflow-hidden rounded-sm border border-border bg-surface-2">
              ...
              <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full">
                {heat.map((v, i) => {
                  ...
                  return (
                    <rect key={i} x={(gx * 100) / GRID} y={(gy * 100) / GRID}
                      width={100 / GRID} height={100 / GRID}
                      fill={heatColor(t)} opacity={alpha} />
                  );
                })}
              </svg>
            </div>
